#!/usr/bin/env python3
"""
03_holographic_dictionary_v3.py - Fase XI: Diccionario Holografico (CORREGIDO, sin manifest.json)

CORRECCIONES vs v2:
    1. Agrupa por (family, d) en vez de solo family.
    2. Cada operador mantiene su d correcto.
    3. En modo hdf5, puede calcular m²L² = Delta(Delta-d) si se pide explicitamente
       via --compute-m2-from-delta (MODO CONTROL).
    4. Genera by_system con claves tipo "ads_d4", "ads_d3", etc.

CORRECCION honestidad:
    - En el flujo "emergente" honesto, este script solo extrae Delta de 
      correladores y construye el atlas de operadores.
    - No calcula m²L² = Delta(Delta-d) dentro del pipeline XI–XII–XII.c.
    - Cualquier uso de la fórmula se limita a modos de control (hdf5+flag) o
      a la comparación teórica en discover_mass_dimension_relation.

ESTRUCTURA DE ENTRADA ESPERADA:
    --data-dir debe apuntar a la carpeta con los .h5 de geometría, p.ej.:

        fase11_output_v2/geometry

    Cada .h5 debe tener al menos:
        - attrs["family"] (opcional, por defecto "unknown")
        - attrs["d"] (opcional, por defecto 4)
        - grupo "boundary/x_grid"
        - opcionalmente attrs["Delta_mass_dict"] (JSON con Delta y m2L2)
"""
