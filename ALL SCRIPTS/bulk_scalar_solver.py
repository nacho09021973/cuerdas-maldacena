...
Módulo para resolver la ecuación de un campo escalar en un fondo holográfico
emergente y extraer un espectro de masas m^2 ("m2L2 experimentales") a partir
de la geometría de Fase XI.

Ecuación considerada (modo estático, sin momento espacial):

    (1/sqrt(-g)) d_z [ sqrt(-g) g^{zz} d_z phi(z) ] - m^2 phi(z) = 0
...
y resolvemos el problema de autovalores:

    L phi_n = m_n^2 phi_n

con condiciones de contorno Dirichlet simples en los extremos del grid.

Este módulo NO usa m^2 L^2 = Delta (Delta - d) en ningún punto del camino
numérico. Cualquier comparación con esa fórmula se puede hacer fuera, como
chequeo teórico.
"""
...
def build_sturm_liouville_operator(geom: BulkGeometry) -> np.ndarray:
    ...
    Devuelve:
        L: matriz NxN tal que L @ phi ≈ m^2 * phi
    """
    z = geom.z
    A = geom.A
    fz = geom.f
...
def solve_mass_spectrum(
    ...
    Returns:
        masses2: array (k,) de m_n^2 (k <= n_eigs)
...
def solve_geometry(
    ...
    masses2, modes = solve_mass_spectrum(geom, n_eigs=n_eigs)
...
def _cli():
    parser = argparse.ArgumentParser(
        description="Resolver espectro escalar en una geometría bulk emergente."
    )
    parser.add_argument(
        "--h5-file",
        type=str,
    ...
    print("======================================================")
    for j, (m2, Delta_uv) in enumerate(zip(result["m2L2"], result["uv_exponents"])):
        if Delta_uv is None:
            print(f"  modo {j}: m^2L^2 ≈ {m2:.6f},   Delta_UV ≈ (no fiable)")
        else:
            print(f"  modo {j}: m^2L^2 ≈ {m2:.6f},   Delta_UV ≈ {Delta_uv:.6f}")
