#!/usr/bin/env python3
"""
00_generate_fase_11_v3.py â€” Fase XI v3: Emergencia AutÃ³noma de GeometrÃ­a

MEJORAS v3:
    - GeneraciÃ³n de mÃºltiples universos por geometrÃ­a base
    - Jitter de parÃ¡metros fÃ­sicos (z_h, d, theta, z_dyn, deformation)
    - CLI escalable: --n-known, --n-test, --n-unknown
    - Backend opcional EMD para familias Lifshitz / hyperscaling
    - Compatible con 01_emergent_geometry_v2.py:
        * boundary: x_grid, temperature, G2_<O>, omega_grid, k_grid, G_R_real, G_R_imag
        * bulk_truth: z_grid, A_truth, f_truth, R_truth (+ attrs z_h, family, d, theta, z_dyn)
        * manifest.json con lista de "geometries"

OBJETIVO:
    El learner NO ve la mÃ©trica ni el solver EMD.
    Solo ve datos CFT en el boundary. El bulk se usa para validaciÃ³n/contratos.
"""
...
class HiddenGeometry:
    """
    GeometrÃ­a del bulk que genera los datos CFT.
    NO se expone al learner (solo a bulk_truth para validaciÃ³n).
    """
    name: str
    family: str           # "ads", "lifshitz", "hyperscaling", "deformed", "unknown"
    category: str         # "known", "test", "unknown"
    d: int                # dimensiÃ³n del boundary (CFT_d)
    z_h: Optional[float] = None  # posiciÃ³n del horizonte (si hay BH)
    theta: float = 0.0           # exponente de hyperscaling
    z_dyn: float = 1.0           # exponente dinÃ¡mico de Lifshitz
    deformation: float = 0.0     # deformaciÃ³n genÃ©rica de A(z)
    ...
    def warp_factor(self, z: np.ndarray) -> np.ndarray:
        """
        A(z) tal que:
            dsÂ² = e^{2A(z)} [ -f(z) dtÂ² + dx_iÂ² ] + dzÂ² / f(z)
        ImplementaciÃ³n toy distinta por familia.
        """
        eps = 1e-6
    ...
    def ricci_scalar(self, z: np.ndarray) -> np.ndarray:
        """
        Escalar de Ricci R(z) aproximado numÃ©ricamente a partir de A(z), f(z).
    ...
    def einstein_tensor_trace(self, z: np.ndarray) -> np.ndarray:
        """
        Traza del tensor de Einstein: G^Î¼_Î¼ = (1 - D/2) R.
        """
        D = self.d + 1
...
# ============================================================
#  GEOMETRÃAS BASE PARA FASE XI
# ============================================================

def get_phase11_geometries() -> List[Tuple[HiddenGeometry, str]]:
    """
    Prototipos de geometrÃ­a (base) para la Fase XI.
    Cada una se clonarÃ¡ con jitter para generar mÃºltiples universos.
    """
...
def jitter_geometry(
    ...
    """
    Copia la geometrÃ­a base con ligeras variaciones de parÃ¡metros.
...
def generate_operators_for_geometry(
    ...
    """
    Espectro toy de operadores escalares O_i con dimensiones Î”_i.
    """
    deltas = np.sort(geo.d / 2 + 0.5 + 2.0 * rng.random(n_ops))
    ops: List[Dict] = []

    for i, Delta in enumerate(deltas):
        # RelaciÃ³n masa-dimensiÃ³n AdS_{d+1}: mÂ²LÂ² = Î”(Î”-d)
...
def generate_boundary_data(
    ...
    """
    Genera datos del boundary (LO ÃšNICO visible al learner).
...
def _ricci_from_A_f(z: np.ndarray, A: np.ndarray, f: np.ndarray, d: int) -> np.ndarray:
    """
    Calcula el escalar de Ricci R para una mÃ©trica estÃ¡tica plano-simÃ©trica,
    usando el mismo esquema que HiddenGeometry.ricci_scalar pero con A,f explÃ­citos.
    """
    D = d + 1
    if len(z) > 1:
        dz = z[1] - z[0]
...
def generate_lifshitz_from_emd(
    ...
    # g_xx ~ z^{2Î¸/d} y g_tt ~ z^{-2z_dyn}, asÃ­ que:
...
def generate_bulk_truth(
    ...
    """
    Genera la "verdad" del bulk para validaciÃ³n.
    ...
    silencioso (con warning) al modo toy analÃ­tico.
    """
    # Por defecto: backend analÃ­tico toy
...
def make_geometry_instance(
    ...
    """
    Crea una copia de `base` con nombre Ãºnico y pequeÃ±os jitters en parÃ¡metros
    segÃºn la familia.
    ...
    # Para las geometrÃ­as AdS puras conocidas (ads5_pure, ads4_pure)
    # queremos al menos una instancia EXACTAMENTE igual al prototipo:
    #   - z_h = None  â†’ T = 0
    #   - family = "ads"
    #   - R(z) constante = -D(D-1)/LÂ² (ver HiddenGeometry.ricci_scalar)
    if (
        base.family == "ads"
    ...
    # --- CÃ“DIGO ORIGINAL DE JITTER -------------------------------------
    params = asdict(base)

    # Nombre Ãºnico: base_category_idx
    params["name"] = f"{base.name}_{category}_{idx:03d}"
    ...
    if z_h is not None and z_h > 0:
        # PerturbaciÃ³n suave multiplicativa
        factor = rng.uniform(0.7, 1.3)
        z_h = float(np.clip(z_h * factor, 0.3, 3.0))
    else:
        # A veces introducimos un pequeÃ±o horizonte en geometrÃ­as sin Ã©l
        if rng.random() < 0.3:
    ...
    # --- Jitters especÃ­ficos por familia ---
    if family == "lifshitz":
        # z_dyn > 1 tÃ­pico
        ...
        # unknown: sÃ³lo tocamos z_h y d (ya hechos arriba)
        pass
...
def main():
    parser = argparse.ArgumentParser(
        description="Fase XI v3: GeneraciÃ³n de datos para emergencia geomÃ©trica (escalable)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="NÃºmero de universos por geometrÃ­a base con category='known'",
    )
    parser.add_argument(
        "--n-test",
        type=int,
        default=10,
        help="NÃºmero de universos por geometrÃ­a base con category='test'",
    )
    parser.add_argument(
        "--n-unknown",
        type=int,
        default=5,
        help="NÃºmero de universos por geometrÃ­a base con category='unknown'",
    )
    parser.add_argument(
        "--use-emd-lifshitz",
        action="store_true",
        help="Usa EMDLifshitzSolver para familias lifshitz/hyperscaling si estÃ¡ disponible",
    )
    ...
    # malla en z comÃºn a todos los universos
    ...
    for idx, (geo, category) in enumerate(geometries):
        ...
        # bulk (solo para validaciÃ³n/contratos)
    ...
    print("\n" + "=" * 70)
    print("âœ“ GENERACIÃ“N FASE XI v3 COMPLETADA")
    print(f"  Manifest: {manifest_path}")
    print(f"  Total:    {len(geometries)} universos")
    print("=" * 70)
    print("\nPrÃ³ximo paso: 01_emergent_geometry_v2.py")
    print("El learner solo verÃ¡ datos del boundary â€” debe descubrir la geometrÃ­a.")
